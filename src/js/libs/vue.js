...
(function(global, factory) {
  ...
  function assertProp(prop, name, value, vm, absent) {
    ...
    if (!valid) {
      warn('Invalid prop: type check failed for prop "' + name + '".' + ' Expected ' + expectedTypes.map(capitalize)
        .join(', ') + ', got ' + Object.prototype.toString.call(value)
        .slice(8, -1) + '.', vm);
      return
    }
  ...
  var initProxy; {
    ...
    var hasProxy = typeof Proxy !== 'undefined' && Proxy.toString()
      .match(/native code/);
    if (hasProxy) {
  ...
  function checkOptionType(vm, name) {
    ...
    if (!isPlainObject(option)) {
      warn(
        ("component option \"" + name + "\" should be an object."), vm);
    }
  }
  ...
  function createPatchFunction(backend) {
    ...
    function emptyNodeAt(elm) {
      return new VNode(nodeOps.tagName(elm)
  ...
  function parseFilters(exp) {
    ...
    function pushFilter() {
      (filters || (filters = []))
      .push(exp.slice(lastFilterIndex, i)
        .trim());
      lastFilterIndex = i + 1;
    }
    if (filters) {
  ...
  var normalize = cached(function(prop) {
    emptyStyle = emptyStyle || document.createElement('div')
      .style;
    prop = camelize(prop);
    if (prop !== 'filter' && (prop in emptyStyle)) {
      return prop
    }
    var capName = prop.charAt(0)
  ...
  function actuallySetSelected(el, binding, vm) {
    ...
    if (isMultiple && !Array.isArray(value)) {
      "development" !== 'production' && warn("<select multiple v-model=\"" + (binding.expression) + "\"> " + "expects an Array value for its binding, but got " + (Object.prototype.toString.call(value)
        .slice(8, -1)), vm);
      return
    }
    var selected, option;
  ...
  var Transition = {
    ...
    render: function render(h) {
      ...
      child.key = child.key == null ? child.isComment ? id + 'comment' : id + child.tag : isPrimitive(child.key) ? (String(child.key)
        .indexOf(id) === 0 ? child.key : id + child.key) : child.key;
  ...
  function isStatic(node) {
    ...
    return !!(node.pre || (!node.hasBindings && // no dynamic bindings
      ...
      !isDirectChildOfTemplateFor(node) && Object.keys(node)
      .every(isStaticKey)))
  }
  ...
  function genKeyFilter(keys) {
    return ("if(!('button' in $event)&&" + (keys.map(genFilterCode)
      .join('&&')) + ")return null;")
  }
  ...
  var unaryOperatorsRE = new RegExp('\\b' + ('delete,typeof,void')
    .split(',')
    .join('\\s*\\([^\\)]*\\)|\\b') + '\\s*\\([^\\)]*\\)');
